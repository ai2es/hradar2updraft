...
#data shape is 3d for 3d conv.
INPUT_SHAPE = (128,128,24,1)
...
#unet params
HP_UNET_DEPTH = hp.HParam('depth_of_unet', hp.Discrete([1,2,3]))
...
HP_BATCHSIZE = hp.HParam('batch_size', hp.Discrete([128,256,512]))
...
def model_fn(hparams, seed,scope=None):
    ...
    model = models.unet_3d(INPUT_SHAPE,kernel_list,OUTPUT_CLASSES,kernel_size=hparams[HP_CONV_KERNEL_SIZE],
...
def run(data, base_logdir, session_id, hparams,cache=None):
    ...
    split_dir2 = split_dir[1].split('logs_3d')
    right = split_dir2[0][:-1] + split_dir2[1]
    left = '/ourdisk/hpc/ai2es/randychase/updraft/paper_results/models_3d/'
...
def run_all(logdir, verbose=False, cache=None):
...
def main(unused_argv):
    ...
    # cache = flags.FLAGS.cache
    cache = None
    # print('removing old logs')
    # shutil.rmtree(logdir, ignore_errors=True)
    print("Saving output to %s." % logdir)
    run_all(logdir=logdir, verbose=True, cache=cache)
