...
#data shape is 2d for 2d conv, channel dim is the height 
...
def run(data, base_logdir, session_id, hparams):
    ...
    ds_train = ds_train.map(lambda x_img,y_img: (reshape_x(x_img), y_img), num_parallel_calls= tf.data.AUTOTUNE)
    ...
    split_dir2 = split_dir[1].split('logs_2d')
    right = split_dir2[0][:-1] + split_dir2[1]
    left = '/ourdisk/hpc/ai2es/randychase/updraft/paper_results/models_2d/'
